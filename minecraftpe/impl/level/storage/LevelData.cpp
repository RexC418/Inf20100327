...
LevelData::LevelData(const LevelData& a2) {
	...
	this->seedLow = a2.seedLow;
	this->seedHigh = a2.seedHigh;
...
LevelData::LevelData(const struct LevelSettings& settings, const std::string& a3, int32_t a4) {
	...
	this->levelName = a3;
	this->ySpawn = 64;
	this->seedLow = static_cast<int32_t>(settings.seed);
	this->seedHigh = static_cast<int32_t>(static_cast<uint64_t>(settings.seed) >> 32);
	this->xSpawn = 128;
	this->zSpawn = 128;
	this->time = 0;
	this->lastPlayed = 0;
...
int64_t LevelData::getSeed() const{
	return static_cast<int64_t>((static_cast<uint64_t>(static_cast<uint32_t>(this->seedHigh)) << 32) | static_cast<uint32_t>(this->seedLow));
}
...
void LevelData::getTagData(const CompoundTag* a2) {
	if(a2) {
		{
		int64_t seed = a2->getLong("RandomSeed");
		this->seedLow = static_cast<int32_t>(seed);
		this->seedHigh = static_cast<int32_t>(static_cast<uint64_t>(seed) >> 32);
	}
...
void LevelData::setSeed(int64_t a2) {
	this->seedLow = static_cast<int32_t>(a2);
	this->seedHigh = static_cast<int32_t>(static_cast<uint64_t>(a2) >> 32);
}
...
void LevelData::setTagData(struct CompoundTag* a2, struct CompoundTag* a3) {
	a2->putLong("RandomSeed", this->getSeed());
...
void LevelData::v1_read(RakNet::BitStream& a2, int32_t a3) {
	...
	a2.Read<int32_t>(this->seedLow);
	this->seedHigh = this->seedLow < 0 ? -1 : 0;
...
void LevelData::v1_write(RakNet::BitStream& a2) {
	a2.Write<int32_t>(this->seedLow); // v1 stored only the platform-sized long
